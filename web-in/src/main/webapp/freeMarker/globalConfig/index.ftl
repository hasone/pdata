...
<div class="main-container">
    ...
    <div class="tools row">
        <div class="col-sm-2">
            <a href="${contextPath}/manage/globalConfig/showAddOrUpdateGlobalConfig.html" class="btn btn-sm btn-danger">
                <i class="fa fa-plus mr-5"></i>新建配置
            </a>
        </div>
        ...
        <div class="col-sm-10 dataTables_filter text-right">
            <div id="searchForm" class="form-inline searchForm" id="table_validate">
                ...
                <div class="form-group form-group-sm">
                    ...
                    <input type="text" class="form-control searchItem" id="configKey" name="configKey">
                </div>
...
<script>
    ...
    var buttonsFormat = function (value, column, row) {
        if (row.deleteFlag == 0) {
            return ["<a class='btn-icon icon-down mr-5' href='javascript:deleteGlobalConfig(" + row.id + ")'>删除</a>",
                "<a class='btn-icon icon-search mr-5' class='btn-icon icon-down mr-5' href='${contextPath}/manage/globalConfig/showAddOrUpdateGlobalConfig.html?id=" + row.id + "'>编辑</a>"];
        }
    }

    var columns = [{name: "name", text: "配置名称", tip: true},
        {name: "configKey", text: "Key值", tip: true},
        {name: "configValue", text: "Value值", tip: true},
    ...
    var action = "${contextPath}/manage/globalConfig/search.html?${_csrf.parameterName}=${_csrf.token}&back=${back!0}";

    require(["common", "bootstrap", "page/list"], function () {
    });
	
	function searchCallback(data){
    	$("#configName").val(data.queryObject.configName);
    	$("#configKey").val(data.queryObject.configKey);
    	$("#pageSize").val(data.queryObject.pageSize);
    	$("#pageNum").val(data.queryObject.pageNum);
    	action = "${contextPath}/manage/globalConfig/search.html?${_csrf.parameterName}=${_csrf.token}";
    }
    ...
    function init(id) {
        $("#ok").on("click", function () {
            $.ajax({
                ...
                url: "${contextPath}/manage/globalConfig/delete.html",
                data: {
                    id: id
                },
