    ...
    <div class="module-header mt-30 mb-20">
        <h3>
            ...
            <a class="btn btn-primary btn-sm pull-right btn-icon icon-back"
               onclick="window.location.href='${contextPath}/manage/enterProductChange/index.html'">返回</a>
        </h3>
    </div>
    <div>
            ...
            <div class="form-group form-group-sm form-inline">
                ...
                <div class="btn-group btn-group-sm template-select">
                    <input type="text" style="width: 0; height:0; opacity: 0;" class="" name="productTemplate"
                           id="productTemplate" 
                           <#if productTemplateEnterpriseMap?? && productTemplateEnterpriseMap.productTemplateId??>value="${productTemplateEnterpriseMap.productTemplateId!}"<#else>value=""</#if>>
                    <button type="button" class="btn btn-default" id="templateName">
                    	<#if productTemplateEnterpriseMap?? && productTemplateEnterpriseMap.productTemplateName??>${productTemplateEnterpriseMap.productTemplateName!}<#else>未选择产品模板</#if>
                    </button>
                    ...
                    <ul class="dropdown-menu templateType">
                        <li data-value=""><a href="#">未选择产品模板</a></li>
                        <#if productTemplates??>
                            <#list productTemplates as item>
                ...
                <form>
                    <div class="form-inline">
                        ...
                        <div class="form-group form-group-sm">
                            ...
                            <input type="text" class="form-control searchItem mobileOnly" name="size"/>
                            <div class="btn-group btn-group-sm">
                                <input type="text" style="width: 0; height:0; opacity: 0;" class="searchItem"
                                       name="unit" value="MB">
                                <button type="button" class="btn btn-default">MB</button>
                                <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown"
                                        aria-haspopup="true" aria-expanded="false">
                                    <span class="caret"></span>
                                    <span class="sr-only">Toggle Dropdown</span>
                    ...
                    <div class="form-inline mt-10">
                        ...
                        <div class="form-group">
                            ...
                            <div class="btn-group btn-group-sm">
                                ...
                                <button type="button" class="btn btn-default" style="width: 110px">全部</button>
                                <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown"
                                        aria-haspopup="true" aria-expanded="false">
                                    <span class="caret"></span>
                                    <span class="sr-only">Toggle Dropdown</span>
...
//        });
    ...
    function getTemplateProducts(){
    	var parent = $(".template-select");
        var templateId = $("input", parent).val();
        ...
        $.ajax({
            ...
            type: "POST",
            url: "${contextPath}/manage/enterProductChange/getTemplateProductsAjax.html",
            data: {
            ...
            success: function (data) {
                if (data.products) {
                	productsChosenByEnterprise(data.products);
                	$("#search-btn2").click();
                }
            },
            error: function () {
    ...
    function addProduct(btnEle) {
        ...
        appendProduct(row);
//        $('a', '.dropdown-menu.templateType li[data-value=""]').click();
        $("#productTemplate").val("");
        $("#productTemplate").parent().find("#templateName").html($(".templateType li[data-value=''] a").html());
        $("#productIds").val(allId);
        $("#search-btn2").click();
    }
    function appendProduct(data) {
        var parent = $("#listData1 tbody");
        var discount = isNaN(discountFun(data["discount"]))?"10.0":discountFun(data["discount"]);
        ...
        if(provinceFlag == "ziying"){
        	...
        	trs = $('<tr data-id="' + data["id"] + '">' +
                ...
                '<td><input class="discount" value='+ discount+'>折' +
    ...
    function tableList(trs) {
        ...
        if(provinceFlag == "ziying"){
        	...
        	trs.each(function () {
	            ...
	            trData["discount"] = $('.discount',this).val();
        	...
        	trs.each(function () {
	            ...
	            if($(this).find(".discount").length !== 0){
	                if (!$(this).find(".discount").val()) {
	                    ...
	                    valid = false;
	                    return false;
	                } else if (!/^(0\.[1-9]|[1-9](\.[0-9])?|10(\.[0])?)$/.test($(this).find(".discount").val())) {
	                    $(this).find(".discount-tip").html('请正确输入折扣信息');
	                    $("#tips").html("请输入(0,10]之间的折扣,只支持1位小数");
	                    $("#tips-dialog").modal('show');
	                    valid = false;
	                    return false;
	                } else {
...
//	                    valid = false;
//	                    return false;
//	                }else if (!/^(0\.[1-9]{1,2}|[1-9](\.[0-9]{1,2})?|10(\.[0]{1,2})?)$/.test($(this).find(".discount").html())) {
//	                    $(this).find(".discount-tip").html('请填写正确折扣');
//	                    $("#tips").html("您填写了错误的折扣");
//	                    $("#tips-dialog").modal('show');
//	                    valid = false;
//	                    return false;
...
//	            }
        ...
        if(valid){
            ...
            for(var i = 0; i < list.length; i++){
                var discount = parseFloat(list[i]["discount"]);//* 10
            ...
            $.ajax({
                ...
                data: {
                    ...
                    productTemplateId: $("#productTemplate").val(),
