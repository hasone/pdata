...
<head>
    ...
    <title>流量平台-用户修改审批列表</title>
    <meta name="keywords" content="流量平台用户修改审批列表"/>
    <meta name="description" content="流量平台 用户修改审批列表"/>
...
<script>
    ...
    var buttons1Format = function (value, column, row) {
        if (row.deleteFlag == 0) {
            return ["<a class='btn-icon icon-syxg mr-5' href='${contextPath}/manage/approval/adminApprovalDetail.html?id=" + row.id + "'>审批</a>"];
        }
    ...
    var columns = [
        ...
        {name: "entCode", text: "企业编码", tip: true},
         {name: "createTime", text: "提交时间", format: "DateTimeFormat"},
         {name: "op1", text: "操作", format: buttons1Format}
       ];

    var action = "${contextPath}/manage/approval/search.html?${_csrf.parameterName}=${_csrf.token}&approvalType=8";
