...
<head>
    ...
    <title>四川流量平台-编辑大转盘</title>
    <meta name="keywords" content="四川流量平台 编辑大转盘"/>
    <meta name="description" content="四川流量平台 编辑大转盘"/>
                ...
                <div class="modal-body" style="padding: 50px;">
                    <div class="form text-center">
                        ...
                        <div class="form-group form-group-sm form-inline mt-10">
                            ...
                            <div class="promote" style="margin-left: 152px; text-align: left;">
                                请输入1-10之间的正整数
                            </div>
                        </div>
                        <div class="form-group form-group-sm form-inline mt-10">
...
<div class="modal fade dialog-sm" id="auth-dialog" data-backdrop="static">
    <div class="modal-dialog">
        <div class="modal-content bd-muted">
            ...
            <div class="modal-body">
                <span class="message-icon message-icon-info"></span>
                <span class="message-content" id="tips">活动保存成功,请点击按钮授权。</span>
            </div>
            <div class="modal-footer">
                <button class="btn btn-warning btn-sm text-center" id="auth-btn">一键授权</button>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal-dialog -->
</div>
...
<script>
    ...
    var action = "${contextPath}/manage/lottery/searchProduct.html?${_csrf.parameterName}=${_csrf.token}";
    var tableData = {};
    var columns = [
    ...
    function init() {
        ...
        $("#sure-btn").on("click", function () {
            window.location.href = "${contextPath}/manage/lottery/index.html";
        });
        
        $("#auth-btn").on("click", function () {
        	var activityId = $("#tips").val();
    		//点击授权
    ...
    function saveData() {
        ...
        <#if allowAllPlatformProduct??  && allowAllPlatformProduct>
            ...
            if (valid) {
                        ...
                        if (!isEmpty) {
                            ...
                            if (product.id) {
                                ...
                                if (product.type == 2) {
                                	...
                                	//虚拟币
                                	item["prizeName"] = $(this).find(".size").val() + "个流量币"; 
                                	item["productSize"] = $(this).find(".size").val();
                                }
                                else {
                ...
                ajaxData("${contextPath}/manage/lottery/saveEditAjax.html", {
    ...
    function sizeFunButNoUnit(size, type) {
        if (size == null) {
            return "";
        }
    ...
    function appendToYY(data) {
        ...
        if (tds.length === 11) {
            ...
            if (data["type"] == "1") {
            	...
            	tds.eq(4).html('<input type="text" class="mobileOnly size">个');
                tds.eq(5).html('-');
            }else {
    ...
    <#if !(cqFlag??&&cqFlag==0)>
        ...
        if (flowType && flowType == 1) {
            tr.append('<td><input type="text" class="size mobileOnly" value="' + sizeFunButNoUnit(
                              record["size"], flowType)                                    + '">MB</td>');

            tr.append('<td' + hidden + '>' + ("-") + '</td>');

        } else if(flowType && flowType == 4){
        	tr.append('<td><input type="text" class="size mobileOnly" value="' + sizeFunButNoUnit(
                              record["size"], flowType)                                    + '">个</td>');
