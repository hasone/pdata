            ...
            <div class="tile-content">
                <div class="row form">
                    ...
                    <#if record.frontImage??>
                        <img src="${contextPath}/manage/mdrc/template/getFile.html?id=${record.id}&filename=${record.frontImage}"
                    ...
                    <div class="form-group">
                        <label>背面预览图：</label>
                    <#if record.rearImage??>
                        <img src="${contextPath}/manage/mdrc/template/getFile.html?id=${record.id}&filename=${record.rearImage}"
                             style="max-width:360px;"/>
                    <#else>
                        -
                    </#if>
                        ...
                        <#list files as file>
                            ...
                            <tr>
                                <td align="center" style="text-align:center;">
                                    <#if isImage(fileType)>
                                        <img src="${contextPath}/manage/mdrc/template/getFile.html?id=${record.id}&filename=${file.name}"
                                ...
                                <td>
                                    <#if file.name??>
                                        <#if (record.frontImage?? && file.name == record.frontImage) || (record.rearImage?? && file.name == record.rearImage)>
                                            -
                                        <#else>
                                            <#if isImage(fileType)>
                                                <a href="${contextPath}/manage/mdrc/template/setCover.html?id=${record.id}&front=${file.name}">设为封面</a>
                                                <a href="${contextPath}/manage/mdrc/template/setCover.html?id=${record.id}&rear=${file.name}">设为背面</a>
                                            </#if>
                                            <a href="${contextPath}/manage/mdrc/template/delete.html?id=${record.id}&filename=${file.name}"
...
<script>
    ...
    function init() {
        $('#file_upload').uploadify({
            ...
            'uploader': '${contextPath}/manage/mdrc/template/upload.html?${_csrf.parameterName}=${_csrf.token}',
            'auto': false,
