...
<head>
    ...
    <title>流量平台-企业开户管理</title>
    <meta name="keywords" content="流量平台 企业开户管理"/>
    <meta name="description" content="流量平台 企业开户管理"/>
...
<div class="main-container">
    ...
    <div class="tools row">
        <div class="col-sm-2">
        <#if submitApprovalFlag?? && submitApprovalFlag == "true">
            <a href="${contextPath}/manage/enterprise/createEnterprise.html">
                <button class="btn btn-sm btn-danger"><i class="fa fa-plus mr-5"></i>新增企业开户</button>
            </a>
        </#if>
        </div>
...
<div class="modal fade dialog-sm" id="confirm-dialog" data-backdrop="static">
    <div class="modal-dialog">
        <div class="modal-content bd-muted">
            ...
            <div class="modal-footer">
                <button class="btn btn-warning btn-sm" id="ok" data-dismiss="modal">确 定</button>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal-dialog -->
</div>
...
//        initTree();
            ...
            $.ajax({
                ...
                if (ret) {
                    if (ret.submitRes == "success") {
                        ...
                        $("#confirm-dialog .message-content").html("提交审批失败！");
    ...
    var buttonsFormat = function (value, column, row) {
        ...
        var links = ["<a class='btn-icon icon-edit mr-5' onclick='editSubmit(" + row.id + ")'>编辑</a>",
            "<a class='btn-icon mr-5' onclick='approvalSubmit(" + row.id + ")'>提交审批</a>"];

        if (submitApprovalFlag == "true") {
            if (row.deleteFlag == 5 || row.deleteFlag == 8) {
    ...
    function approvalSubmit(entId){
        $.ajax({
            type: "POST",
            url: "${contextPath}/manage/approval/checkEnterpriseApproval.html?${_csrf.parameterName}=${_csrf.token}",
            data: {
                entId: entId,           
            },
            ...
            success: function (res) {
                if (res.msg && res.msg == "success") {
                    window.location.href="${contextPath}/manage/approval/submitEntApproval.html?entId=" + entId + "&type=1";                    
                }else{
                    $("#confirm-dialog .message-content").html("请勿重复提交！");
                    $("#confirm-dialog").modal("show");                              
                }                       
            },
            error: function () {
    ...
    function editSubmit(entId){
        $.ajax({
            type: "POST",
            url: "${contextPath}/manage/enterprise/checkEnterpriseEdit.html?${_csrf.parameterName}=${_csrf.token}",
            data: {
                entId: entId,           
            },
            dataType: "json", //指定服务器的数据返回类型，
            success: function (res) {
                if (res.msg && res.msg == "success") {
                    window.location.href="${contextPath}/manage/enterprise/editEnterprise.html?entId=" + entId;                    
                }else{
                    $("#confirm-dialog .message-content").html("该记录已提交审批！");
                    $("#confirm-dialog").modal("show");                              
                }                       
            },
            error: function () {
                $("#confirm-dialog .message-content").html("无法编辑该企业信息！");
                $("#confirm-dialog").modal("show");
            }
        });
    }
