    ...
    <div class="modal-dialog" style="width:525px;">
        <div class="modal-content bd-muted">
            <div class="modal-header bg-muted bd-muted">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                        aria-hidden="true">&times;</span></button>
                <h5 class="modal-title">暂停操作</h5>
            </div>
            <div class="modal-body" style="padding-top:20px;">
            <div style="width:80%;margin:0 auto;">
            <div class="form-group form-inline" id="reason">
            	<label>暂停原因</label>
            	<select name="reason" class="form-control">
	            	<option value='-1'>请选择原因</option>
	            	<option value='1'>集团通报</option>
	            	<option value='2'>省公司通报</option>
	            	<option value='3'>合同到期</option>
	            	<option value='4'>客户需求</option>
	            	<option value='5'>其他</option>
            	</select>
            	<span id="reason-tip"></span>
            </div>
            	<div class="form-group form-inline">
            	<label>备注</label>
                <textarea rows="4" maxlength="300" id="opDesc" name="opDesc" class="hasPrompt form-control" style="width: 60%; resize:none;"></textarea>
                <span class="help-block"><span style="color:red;margin-left:23%;">*</span>必填：备注信息长度不超过300个字符。</span>
                <span id="open-tip" style="margin-left:23%;"></span>
                </div>
             </div>
             </div>
            <div class="modal-footer">
...
<script>
    ...
    var columns = [{name: "name", text: "企业名称", tip: true, format: nameFormat},
        ...
        {name: "ecPrdCode", text: "集团产品号码", tip: true, format: ecPrdCode},
        {name: "status", text: "产品状态", format: statusFormat},
    ...
    function init() {
        ...
        $("#open-ok").on("click", function () {
            ...
            if($("#reason select:visible").length > 0 && reason == '-1'){
            	$("#reason-tip").html("请选择理由").css("color","red");
            }
            if(isBlank(opDesc)){
    ...
    function showOpDesDialog(id,status){
        $("#open-tip").html("");
        if(status == 2){
        	$(".modal-dialog .modal-title").html("暂停操作");
        	$("#reason label").html("暂停原因");
        	$("#reason select").val("-1");
        	$("#reason").show();
        }else if(status == 3){
        	$(".modal-dialog .modal-title").html("关闭操作");
        	$("#reason label").html("关闭原因");
        	$("#reason select").val("-1");
        	$("#reason").show();
        }else if(status == 0){
        	$(".modal-dialog .modal-title").html("恢复操作");
        	$("#reason select").val("-1");
        	$("#reason").hide();
        }
