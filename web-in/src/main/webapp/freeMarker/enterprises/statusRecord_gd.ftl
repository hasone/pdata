...
<head>
    ...
    <title>流量平台-企业状态操作记录</title>
    <meta name="keywords" content="流量平台 企业状态操作记录"/>
    <meta name="description" content="流量平台 企业状态操作记录"/>
...
<div class="main-container">
    ...
    <div class="tools row text-right">
        <div class="col-lg-12 dataTables_filter text-right">
        	<a class="btn btn-primary btn-sm pull-right btn-icon icon-back" onclick="javascript:history.go(-1)">返回</a>
    ...
    <script>
        var opTypeFormat = function (value, column, row) {
        	if(row.nowStatus == 0){
        		return "开通";
        	}else if(row.nowStatus == 2){
        		return "暂停";
        	}else if(row.nowStatus == 3){
        		return "关闭";
        	}else{
        		return "未知操作";
        	}
       ...
       var buttonsFormat = function (value, column, row) {
       		return "<a href='javascript:void(0)' class='btn-icon icon-detail mr-5' onclick='showStatusChangeDescAjax(" + row.id + ")'>查看</a>";
        }
        ...
        var reasonFormat = function (value, column, row) {
			if (!value) {
				return "-";
			}
			return value;
        ...
        var columns = [
        	{name: "createTime", text: "企业状态操作时间", format: "DateTimeFormat"},        	
            {name: "operatorName", text: "操作用户", tip: true},
            {name: "operatorRole", text: "用户角色", tip: true},
            {name: "opType", text: "操作类型", tip: true, format:opTypeFormat},  
            {name: "reason", text: "原因类型", tip: true, format:reasonFormat}, 
            {name: "opDesc", text: "备注信息", format:opDescFormat}];
		
		var action = "${contextPath}/manage/enterprise/statusSearch.html?${_csrf.parameterName}=${_csrf.token}&&entId=${entId!}";
    ...
    function showStatusChangeDescAjax(id){
     	$.ajax({
                type: "POST",
                url: "${contextPath}/manage/enterprise/showStatusChangeDescAjax.html?${_csrf.parameterName}=${_csrf.token}",
