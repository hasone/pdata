...
<head>
    ...
    <link rel="stylesheet"
          href="${contextPath}/assets_individual/lib/daterangepicker/daterangepicker.css"/>
...
<div class="main-container">
    ...
    <div class="tile mt-30">
        ...
        <form id="dataForm" class="form mt-20 row"
              action="${contextPath}/manage/historyEnterprise/saveChangeEnterprise.html?${_csrf.parameterName}=${_csrf.token}"
            ...
            <input type="hidden" name="interfaceFlag" id="interfaceFlag"
                   value="${enterprises.interfaceFlag!}"/>
            ...
            <div style="overflow: hidden">
                <div class="col-md-6">
                    <div class="form-group form-group-sm form-inline">
                        ...
                        <input type="text" name="name" id="name" value="${(enterprises.name)!}"
                               class="form-control edit-input hidden" maxLength="64">
                        <a href="javascript:void(0)"
                           class="primary-text ml-20 client-edit-btn">修改</a>
                        <a href="javascript:void(0)"
                           class="primary-text ml-20 client-save-btn hidden">保存</a>
                    </div>
                    ...
                    <div class="form-group form-group-sm form-inline">
                        <label>企业编码:</label>
                        <span class="editable-text">${(enterprises.code)!}</span>
                        <input type="text" name="code" id="code" value="${(enterprises.code)!}"
                               class="form-control edit-input hidden">
                    <#if flag==0>
                        <a href="javascript:void(0)"
                           class="primary-text ml-20 client-edit-btn">修改</a>
                        <a href="javascript:void(0)"
                    ...
                    <div class="form-group">
                        ...
                        <input type="hidden" name="customerTypeId" id="customerTypeId"
                               value="${enterprises.customerTypeId!}">
                    ...
                    <div class="form-group">
                        <label>支付类型：</label>
                        <input type="hidden" name="payTypeId" id="payTypeId"
                               value="${enterprises.payTypeId!}">
                    ...
                    <div class="form-group form-group-sm form-inline">
                        ...
                        <span class="editable-text"
                              data-value="${(enterprises.discount)!}">${(enterprises.discountName)!}</span>
                        <div class="btn-group btn-group-sm edit-input hidden">
                            <input name="discount" id="discount" value="${(enterprises.discount)!}"
                                   style="width: 0; height: 0;padding: 0; opacity: 0;">
                            <button type="button"
                                    class="btn btn-default btn-select">${(enterprises.discount)!}</button>
                            <button type="button"
                                    class="btn btn-default dropdown-toggle btn-select-color"
                            ...
                            <#if discountList??>
                                <#list discountList as discount>
                                    <li data-value="${discount.id!}"><a
                                            href="#">${discount.name!}</a></li>
                                </#list>
                            </#if>
                            </ul>
                        </div>
                        <#--增加是否可以修改折扣的标识，后台根据平台进行赋值-->
                        <#if discountModify??&&discountModify>
                        <a href="javascript:void(0)"
                           class="primary-text ml-20 client-edit-btn">修改</a>
                        <a href="javascript:void(0)"
                           class="primary-text ml-20 client-save-btn hidden">保存</a>
                    ...
                    <div class="form-group form-group-sm form-inline">
                        ...
                        <span class="editable-text"
                              data-value="${(giveMoneyEnter.giveMoneyId)!}">${(giveMoneyEnter.giveMoneyName)!}</span>
                        <div class="btn-group btn-group-sm edit-input hidden">
                            <input name="giveMoneyId" id="giveMoneyId"
                                   value="${(giveMoneyEnter.giveMoneyId)!}"
                                   style="width: 0; height: 0;padding: 0; opacity: 0;">
                            <button type="button"
                                    class="btn btn-default btn-select">${(giveMoneyEnter.giveMoneyId)!}</button>
                            <button type="button"
                                    class="btn btn-default dropdown-toggle btn-select-color"
                                    data-toggle="dropdown"
                                    aria-haspopup="true" aria-expanded="false">
                            ...
                            <#if giveMoneyList??>
                                <#list giveMoneyList as giveMoney>
                                    <li data-value="${giveMoney.id!}"><a
                                            href="#">${giveMoney.name!}</a></li>
                                </#list>
                            </#if>
                            </ul>
                        </div>
                ...
                <div class="col-md-6">
                    <div class="form-group form-group-sm form-inline">
                        ...
                        <span class="editable-text">${(enterprises.startTime?datetime)!}
                            ~ ${(enterprises.endTime?datetime)!}</span>
                        <span class="edit-input hidden date-picker"></span>
                        <input type="hidden" id="startTime" name="startTime"
                               value="${(enterprises.startTime?datetime)!}"/>
                        <input type="hidden" id="endTime" name="endTime"
                               value="${(enterprises.endTime?datetime)!}"/>
                        <a href="javascript:void(0)"
                           class="primary-text ml-20 client-edit-btn">修改</a>
                        <a href="javascript:void(0)"
                           class="primary-text ml-20 client-save-btn hidden">保存</a>
                    ...
                    <div class="form-group form-group-sm form-inline">
                        ...
                        <span class="editable-img">
                            <img src="${contextPath}/manage/enterprise/getImage.html?enterpriseId=${enterprises.id}&type=businessLicence"
                                 class="prev-img" style="max-width: 130px">
                        </span>
                        <span class="file-box" style="vertical-align: top">
                            <input class="file-helper imge-check" type="file" name="licenceImage"
                                   id="licenceImage" value="${(enterpriseFile.businessLicence)!}"
                                   accept="image/*">
                            <a href="javascript:void(0)"
                               class="primary-text ml-20 client-upload-btn">修改</a>
                        </span>
                    ...
                    <#if enterprises.licenceStartTime??&&enterprises.licenceEndTime??>
                        <span class="editable-text">${(enterprises.licenceStartTime?datetime)!}
                            ~ ${(enterprises.licenceEndTime?datetime)!}</span>
                        <span class="edit-input hidden date-picker"></span>
                        <input type="hidden" id="licenceStartTime" name="licenceStartTime"
                               value="${(enterprises.licenceStartTime?datetime)!}"/>
                        <input type="hidden" id="licenceEndTime" name="licenceEndTime"
                               value="${(enterprises.licenceEndTime?datetime)!}"/>
                        <a href="javascript:void(0)"
                           class="primary-text ml-20 client-edit-btn">修改</a>
                        <a href="javascript:void(0)"
                           class="primary-text ml-20 client-save-btn hidden">保存</a>
                    ...
                    <div class="form-group form-group-sm form-inline">
                        ...
                        <a id="authorization" class="file-text primary-text"
                           href="javascript:void(0)" onclick="downLoad(this)"
                    ...
                    <div class="form-group" id="identification">
                        <div class="form-group form-group-sm form-inline">
                            <label style="vertical-align: top"
                                   id="identification">企业管理员身份证（正面）:</label>
                        <span class="editable-img">
                            <img src="${contextPath}/manage/enterprise/getImage.html?enterpriseId=${enterprises.id}&type=identification"
                                 class="prev-img" style="max-width: 130px">
                        </span>
                        </div>
                        ...
                        <div class="form-group form-group-sm form-inline">
                            <label style="vertical-align: top"
                                   id="identificationBack">企业管理员身份证（反面）:</label>
                        <span class="editable-img">
                            <img src="${contextPath}/manage/enterprise/getImage.html?enterpriseId=${enterprises.id}&type=identificationBack"
                                 class="prev-img" style="max-width: 130px">
                        </span>
                        </div>
                    </div>
                ...
                <#if enterpriseFile?? && enterpriseFile.customerfileName??>
                    <div class="form-group form-group-sm form-inline">
                        ...
                        <a id="customerFile" class="file-text primary-text"
                           href="javascript:void(0)" onclick="downLoad(this)"
                ...
                <#if enterpriseFile?? && enterpriseFile.contractName??>
                    <div class="form-group form-group-sm form-inline">
                        ...
                        <a id="contract" class="file-text primary-text" href="javascript:void(0)"
                ...
                <#if enterpriseFile?? && enterpriseFile.imageName??>
                    <div class="form-group form-group-sm form-inline">
                        <label style="vertical-align: top" id="image">审批截图:</label>
                            <span class="editable-img">
                                <img src="${contextPath}/manage/enterprise/getImage.html?enterpriseId=${enterprises.id}&type=image"
                                     class="prev-img" style="max-width: 130px">
                            </span>
                            <span class="file-box" style="vertical-align: top">
                                <input class="file-helper imge-check" type="file" name="image"
                                       id="image"
            ...
            <div class="form-group-sm mt-20" style="padding: 0 15px;">
...
<script>
    ...
    function init() {
        ...
        $("#history-btn").on("click", function () {
            window.location.href =
                    "${contextPath}/manage/historyEnterprise/index.html?entId=${enterprises.id!}";
        });
    ...
    function downLoad(obj) {
        ...
        location.href =
                "${contextPath}/manage/enterprise/downloadFile.html?enterpriseId=${enterprises.id}"
                + "&&type=" + type;
    }
