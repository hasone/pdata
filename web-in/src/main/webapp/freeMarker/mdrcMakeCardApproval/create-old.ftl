...
<head>
    ...
    <title>流量平台-营销卡制卡申请</title>
    <meta name="keywords" content="流量平台 制卡申请"/>
    <meta name="description" content="流量平台 制卡申请"/>
    ...
    <div class="module-header mt-30 mb-20">
        <h3>新建营销卡数据
            <a class="btn btn-primary btn-sm pull-right btn-icon icon-back"
               onclick="javascript:window.location.href='${contextPath}/manage/mdrc/cardmake/index.html'">返回</a>
        </h3>
    </div>
                ...
                <form action="${contextPath}/manage/mdrc/batchconfig/save.html?${_csrf.parameterName}=${_csrf.token}" method="post"
                    ...
                    <div class="col-md-12 col-md-offset-1">
                        <div class="form-group">
                            ...
                            <div class="prompt">
                            	卡名称格式:企业名称+流量卡额度+年月日(20170101)
                            </div>
                        </div>

                        <div class="form-group">
                            <label for="operatorSelect" required>制卡商：</label>
                        ...
                        <div class="form-group">
                            <label for="themeSelect" required>卡面主题：</label>
                        ...
                        <div class="form-group">
                            <label for="template" required>模板名称：</label>
                            <div class="btn-group">
                            <#if templateMap??>
                                <#list templateMap?keys as key>
...
<script>
    ...
    function listeners(){
    	$("#tips-btn").on("click", function () {
    		//0：成功；1:参数异常，2：制卡商已经被删除；3内部错误
    		var failType = $("#tips").val();
    		if(failType && failType == 2){
    			//重新请求获取制卡商
    			getCardMakers();
    		}
    	});
    ...
    function getCardMakers(){
    	$.ajax({
            ...
            url: "${contextPath}/manage/mdrc/cardmake/getCardMakersAjax.html",
            type: "POST",
            dataType: "json",
            async: true,
            success: function (data) {
            	//怎么替换掉select框里面的下拉选项
            	if(data&&data.cardmakerList){
	            	var cardmakerList = JSON.parse(data.cardmakerList);
	            	$('#cardmakerId').children('option').remove();
	            	$('#cardmakerId').append('<option value="-1">---请选择---</option>');
	            	$.each(cardmakerList,function(index,item){
	            		$('#cardmakerId').append('<option value='+item.id+'>'+item.name+'</option>');
	            	});
            	}else{
            		$('#cardmakerId').children('option').remove();
	            	$('#cardmakerId').append('<option value="-1">---请选择---</option>');
...
//        }
    ...
    function submitApproval(data) {
        $.ajax({
            ...
            url: "${contextPath}/manage/mdrc/cardmake/submitAjax.html",
            type: "POST",
            dataType: "json",
            async: true,
            data: data,
    ...
    function doSave(data) {
        $.ajax({
            ...
            url: "${contextPath}/manage/mdrc/cardmake/saveAjax.html",
            type: "POST",
            dataType: "json",
            async: true,
            data: data,
