...
<head>
    ...
    <title>流量平台-制卡申请列表</title>
    <meta name="keywords" content="流量平台 制卡申请列表"/>
    <meta name="description" content="流量平台 制卡申请列表"/>
...
<script>
    ...
    var buttonsFormat = function (value, column, row) {
        ...
        if(row.cardmakeStatus==0){
            return ["<a class='btn-icon icon-detail mr-5' href='${contextPath}/manage/mdrc/batchconfig/preMakeCard.html?requestId=" + row.id + "' >生成卡数据</a>"];
        }
        else{
            return ["<a class='btn-icon icon-detail mr-5' href='${contextPath}/manage/mdrc/batchconfig/makecardDetail.html?requestId=" + row.id + "' >详情</a>"];
        }
    }
    ...
    var cardmakeStatusFormat = function (value, column, row) {
        if(row.cardmakeStatus==0){
            return "未处理";
        }
        if(row.cardmakeStatus==1){
            return "已处理";
        }
    };
    ...
    var columns = [
        ...
        {name: "customerManagerName", text: "申请用户",tip:true},
        {name: "customerManagerMobile", text: "联系方式"},
        {name: "amount", text: "制卡数量"},
        {name: "configName", text: "卡名称",tip:true},
        {name: "templateName", text: "模板名称",tip:true},
        {name: "cardmakerName", text: "制卡商"},
        {name: "cardmakeStatus", text: "状态", format: cardmakeStatusFormat},
        {name: "op", text: "操作", format: buttonsFormat}
    ];
    var action = "${contextPath}/manage/mdrc/batchconfig/cardmakeIndexSearch.html?${_csrf.parameterName}=${_csrf.token}";
</script>
</body>
</html>