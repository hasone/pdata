...
<script>
    ...
    columns.push({name: "op", text: "操作", format: function(value, column, row){
        return "<a href='${contextPath}/cq/product/productAccountDetail.html?productCode=" + row.productCode + "&entId=${enterprise.id!}' class='btn-icon icon-detail mr-5'> 详情</a>"
    }})

    var action = "${contextPath}/cq/product/searchOrderList.html?${_csrf.parameterName}=${_csrf.token}";
