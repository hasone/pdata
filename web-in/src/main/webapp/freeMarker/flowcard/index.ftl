...
<div class="main-container">
    ...
    <div class="tools row">
        ...
        <div class="col-sm-2">
            <a href="${contextPath}/manage/flowcard/create.html">
                <button class="btn btn-sm btn-danger"><i class="fa fa-plus mr-5"></i>新增流量券</button>
            </a>
        </div>
...
<script>
    ...
    var buttons1Format = function (value, column, row) {
        ...
        if (row.status == 0 || row.status == 7) {
            if(approvalFlag == "true"){
                if(row.download && row.download==1){
                    return ["<a class='btn-icon icon-detail mr-5' href='${contextPath}/manage/flowcard/detail.html?activityId=" + row.activityId + "'>详情</a>",
                        '<a class="btn-icon icon-detail mr-5" href="javascript:submitActivityApproval(\'' + row.activityId + '\')">提交审批</a>'];
                }else{
                    return ["<a class='btn-icon icon-edit mr-5' href='${contextPath}/manage/flowcard/edit.html?id=" + row.id + "'>编辑</a>",
                        "<a class='btn-icon icon-detail mr-5' href='${contextPath}/manage/flowcard/detail.html?activityId=" + row.activityId + "'>详情</a>",
                        '<a class="btn-icon icon-detail mr-5" href="javascript:submitActivityApproval(\'' + row.activityId + '\')">提交审批</a>'];
                }
            }
            else{
            ...
            return ["<a class='btn-icon icon-down mr-5' href='${contextPath}/manage/flowcard/offShelf.html?activityId=" + row.activityId + "'>下架</a>",
                    "<a class='btn-icon icon-detail mr-5' href='${contextPath}/manage/flowcard/detail.html?activityId=" + row.activityId + "'>详情</a>"];
        }
        else {
            return ["<a class='btn-icon icon-detail mr-5' href='${contextPath}/manage/flowcard/detail.html?activityId=" + row.activityId + "'>详情</a>"];
    ...
    var action = "${contextPath}/manage/flowcard/search.html?${_csrf.parameterName}=${_csrf.token}";
    ...
    function init() {
        $("#sure").on("click", function () {
            ...
            $.ajax({
                ...
                url: "${contextPath}/manage/flowcard/submitActivityApprovalAjax.html",
                data: {
    ...
    function onShelf(activityId) {
        $.ajax({
            ...
            type: "post",
            url: "${contextPath}/manage/flowcard/onShelf.html",
            data: {
                activityId: activityId
            },
