...
<head>
    ...
    <meta name="keywords" content="流量平台 流量券详情"/>
    <meta name="description" content="流量平台 流量券详情"/>
...
<#if opinions??>
                ...
                <#if activity.status?? && (activity.status == 0 || activity.status == 1 || 
                    ...
                    <div class="form-group">
                        <span class="detail-label">活动规模（估计）:</span>
                            <span class="detail-value">${(activityInfo.price/100.0)?string("#.##")}元=》
                                <#if activityInfo.totalProductSize??>
                                    <#if ((activityInfo.totalProductSize)<1024)>
                                        <span>${(activityInfo.totalProductSize)?string("#.##")}KB</span>
                                    </#if>
                                    <#if ((activityInfo.totalProductSize)>=1024) && ((activityInfo.totalProductSize?number)<1024*1024)>
                                        <span>${((activityInfo.totalProductSize)/1024.0)?string("#.##")}MB</span>
                                    </#if>
                                    <#if ((activityInfo.totalProductSize)>=1024*1024)>
                                        <span>${((activityInfo.totalProductSize)/1024.0/1024.0)?string("#.##")}GB</span>
                                    </#if>
                                </#if>
                    ...
                    <div class="form-group">
                        <span class="detail-label">活动规模（当前）:</span>
                            <span class="detail-value">${(currentActivityInfo.money/100.0)?string("#.##")}元=》
                            <#if currentActivityInfo.flow??>
                                ...
                                <#if ((currentActivityInfo.flow)>=1024) && ((currentActivityInfo.flow)<1024*1024)>
                                    <span>${((currentActivityInfo.flow)/1024.0)?string("#.##")}MB</span>
                                </#if>
                                <#if ((currentActivityInfo.flow)>=1024*1024)>
                                    <span>${((currentActivityInfo.flow)/1024.0/1024.0)?string("#.##")}GB</span>
                                </#if>
                            </#if>
                          	流量=》${currentActivityInfo.count!}份奖品=》${currentActivityInfo.userCount!}中奖用户</span>
                    </div>
                    -->
                    <a class="btn btn-sm btn-warning"
                       href="${contextPath}/manage/flowcard/downloadPhones.html?activitiesId=${activity.activityId!}">下载手机号</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="mt-30">
        <a class="btn btn-sm btn-warning"
           href="${contextPath}/manage/flowcard/records.html?activityId=${activity.activityId}">查看赠送用户信息</a>
    </div>
