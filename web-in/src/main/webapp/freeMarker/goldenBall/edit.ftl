...
<head>
    ...
    <title>编辑大转盘活动</title>
    <meta name="keywords" content="编辑大转盘活动"/>
    <meta name="description" content="编辑大转盘活动"/>
...
<div class="page-header">
    <h1>编辑大转盘活动 <a class="btn btn-white pull-right" href="${contextPath}/manage/lotteryActivity/index.html">返回</a></h1>
</div>
        ...
        <div class="tile">
            ...
            <div class="tile-content">
                ...
                <div class="form-group">
                    ...
                    <input type="button" id="authorize" value="一键授权" onclick="startAuthorize();" style="display:none;"/>
                </div>
                <div class="form-group">
                    <label>流量类型：</label>
                ...
                <div class="form-group">
                    ...
                    <span id="activeTime">
                        <input type="text" name="startTime" id="startTime" readonly="readonly"
                               value="${(activity.startTime)?date}" required>&nbsp;至&nbsp;<input type="text"
                                                                                                 id="endTime"
                                                                                                 name="endTime"
                                                                                                 value="${(activity.endTime)?date}"
                                                                                                 readonly="readonly"
                                                                                                 required>
                    </span>
                </div>
                ...
                <div class="form-group"
                <label>活动规则：</label>
                <textarea id="activityRule" name="activityRule" style="width:400px;height:200px;"
                          placeholder="活动规则说明。。。">${(activity.rule)!}</textarea>
            </div>
                    ...
                    <tr>
                        <td>
                            <input type="text" class="rankName" name="rankName" required
                                   data-bind="value: awardsText[$index()]" readonly="readonly" style="width:70px;">
                        </td>
                        <td>
                            <select name="productName" class="productName"></select>
                        </td>
                        <td>
                            <input name="prizeName" class="prizeName" data-bind="value: prizeName" readonly
                                   style="width:80px;"></input>
                        </td>
                        <td>
                            <input type="number" name="balance" class="balance" style="width:90px;"
                                   data-bind="value: balance" required required data-rule-number="true" readonly>
                        </td>
                        <td>
                            <input type="number" data-bind="value: count" class="count" name="count" min="0"
                                   max="999999" value="0" required data-rule-number="true" style="width: 80px;">
                        </td>
                        <td data-bind="visible: $parent.probabilityDisplay">
                            <input type="text" data-bind="value: probability" class="probability" name="probability"
                                   min="0" max="1" required style="width: 60px;">
                        </td>
                        <td>
...
<script src="../../manage/assets/js/tooltip.js"></script>
<script src="../../manage/assets/js/daterangepicker/moment.min.js"></script>
<script src="../../manage/assets/js/daterangepicker/jquery.daterangepicker.js"></script>
<script>
    var MAXAWARDNUM = 6;
    ...
    function activePreAward(ele) {
        ...
        preAward.find(".enterpriseName").removeAttr("disabled");
    ...
    var viewModel = {
        ...
        probabilityType: ko.observable("${activity.probabilityType}"),
    ...
    $(function () {
        ...
        enterpriseChoiceListener($("#enterpriseName"), function () {
            for (var i in awards) {
                addAward(awards[i]);
            }
        });
    ...
    function addAwardListener() {
        ...
        $(".add-award").on("click", function () {
            ...
            if (awards.length > 0) {
                ...
                if (Number(probabilityType) == 0 && !probalility.match(/^\d+(\.\d+)?$/)) {
                    showTip("奖项信息设置错误！");
                    return false;
                }
                ...
                pre_award.find(".count").attr("aria-required", "true");
                pre_award.find(".count").attr("aria-invalid", "false");
                pre_award.find(".count").attr("aria-describedby", "count-error");
    ...
    function addAward(award) {
        ...
        viewModel.awards.push(award);

        appendProducts();

        var ele = $(".productName:last");
        ele.val(award.productId);
        ele.on("change", function () {
            var productId = $(this).val();
            var productCode = $(this).data("code");
            var parent = $(this).parents("tr");
            ...
            var target = $(".balance", parent);
            //getBalance(enterCode, productId, target);
            initBalance(enterCode, productId, target);
        });
    }
    ...
    function formValidate() {
        $("#dataForm").validate({
            errorPlacement: function (error, element) {
                ...
                //$("#award-count-error").html("");
                //$("#award-count-error").append(error);
            ...
            rules: {
                name: {
                    ...
                    cmaxLength: 64,
                    remote: {
                        url: "checkActivityName.html?id=${(activity.id)!}",
                        data: {
    ...
    function saveForm() {
        $(".save-btn").on("click", function () {
            ...
            award_ele.each(function (index) {
                ...
                if (Number(obj.productId) > 0 && Number(obj.count) >= 0) {
                    if ((Number(params.probabilityType) == 0)) {//手动设置奖项概率
                        if (obj.probalility == '') {
                            showTip("奖项信息不完整！");
                            flag = false;
                            return false;
                        }
                        if (!obj.probability.match(/^\d+(\.\d+)?$/)) {
                            showTip("奖项信息设置错误！");
                            flag = false;
                            return false;
                        }
                    } else {//自动设置奖项概率
                        if (!obj.count.match(/^\d+$/) || Number(obj.count) > Number($(".balance", this).val())) {
                            showTip("奖项信息设置错误！");
                            flag = false;
                            return false;
                        }
    ...
    */
     ...
     * 获取企业的产品
     */
    function getProducts(enterId, cback) {
        $.ajax({
            beforeSend: function (request) {
                var token1 = $(window.parent.document).find("meta[name=_csrf]").attr("content");
                var header1 = $(window.parent.document).find("meta[name=_csrf_header]").attr("content");
            ...
            pnEle.empty();
            pnEle.append("<option value='0'>请选择</option>");
            for(var i in products){
    ...
    function initBalance(enterCode, productID, ele) {
        var parent = ele.parents("tr");
        if (enterCode && productID) {
            $.ajax({
                ...
                if (data && data.message) {
                    ...
                    var onUsed = 0;
                    var elements = $(".award-table tbody tr");
                    ...
                    var beles = elements.find("td input[name='balance']")
                    var celes = elements.find("td input[name='count']")
                    for (var i = 0; i < beles.length; i++) {
                        for (var j = 0; j < i; j++) {
                            onUsed += Number(celes[j].value);
                        }
                        beles[i].value = balance - onUsed;
                    }

                    $(".award-table tbody tr").not(":last").each(function (index) {
                        $(this).find(".enterpriseName").attr("disabled", "disabled");
                        $(this).find(".productName").attr("disabled", "disabled");
                        $(this).find(".count").attr("disabled", "disabled");
                        $(this).find(".delBtn").css("display", "none")
                    });
                    var balanceEle = ele.parents("tr").find(".balance").last();
                    var countEle = ele.parents("tr").find(".count").last();
                    countEle.attr("max", balanceEle.val());
                    countEle.attr("data-msg-max", "请填写1-" + balanceEle.val() + "之间的数字");
                    countEle.attr("data-msg-min", "请填写1-" + balanceEle.val() + "之间的数字");
                }
            }).fail(function () {
                showTip("网络错误或请求超时");
            });
