...
<div class="main-container">
    ...
    <div class="tools row ">
        <div class="col-sm-2">
            <a href="${contextPath}/manage/goldenBall/create.html">
                <button class="btn btn-danger"><i class="fa fa-plus mr-5"></i>新增砸金蛋</button>
            </a>
        </div>
    ...
    var buttonsFormat = function (value, column, row) {
        ...
        if (row.status == 0 || row.status == 7) {
            if(approvalFlag=="true"){
                return ["<a class='btn-icon icon-detail mr-5' href='${contextPath}/manage/goldenBall/detail.html?id=" + row.id + "'>详情</a>",
                    '<a class="btn-icon icon-detail mr-5" href="javascript:submitActivityApproval(\'' + row.activityId + '\')">提交审批</a>'];
            }
            else{
                return ["<a class='btn-icon icon-detail mr-5' href='${contextPath}/manage/goldenBall/detail.html?id=" + row.id + "'>详情</a>",
                    "<a href='javascript:void(0)' class='btn-icon icon-online mr-5' data-url='${contextPath}/manage/goldenBall/upshelfAjax.html' data-id='" + row.id + "' onclick='sendPost(this);'>上架</a>"];
            }
        }
        else if (row.status == 6) {
            return ["<a href='javascript:void(0)' class='btn-icon icon-online mr-5' data-url='${contextPath}/manage/goldenBall/upshelfAjax.html' data-id='" + row.id + "' onclick='sendPost(this);'>上架</a>",
                "<a class='btn-icon icon-detail mr-5' href='${contextPath}/manage/goldenBall/detail.html?id=" + row.id + "'>详情</a>"];
        }
        else if (row.status == 2) {
            return ["<a href='javascript:void(0)' class='btn-icon icon-online mr-5' data-url='${contextPath}/manage/goldenBall/downshelfAjax.html' data-id='" + row.id + "' onclick='sendPost(this);'>下架</a>",
                "<a class='btn-icon icon-detail mr-5' href='${contextPath}/manage/goldenBall/detail.html?id=" + row.id + "'>详情</a>"];
        } else {
            return ["<a class='btn-icon icon-detail mr-5' href='${contextPath}/manage/goldenBall/detail.html?id=" + row.id + "'>详情</a>"];
        }
...
//        {name: "prizeCount", text: "中奖人次"},
        {name: "op", text: "操作", format: buttonsFormat}];

    var action = "${contextPath}/manage/goldenBall/search.html?${_csrf.parameterName}=${_csrf.token}";
    ...
    function init() {
        $("#sure").on("click", function () {
            ...
            $.ajax({
                ...
                url: "${contextPath}/manage/goldenBall/submitActivityApprovalAjax.html",
                data: {
