...
<#if files??>
    <#list files as file>
        <tr id="${configId}">
            ...
            <td><a target="_blank" id="download-btn"
                   href="${contextPath}/manage/mdrc/download/file.html?configId=${configId}&fileName=${encoder.encode(file.name, 'utf-8')}&templateId=${templateId}">下载</a>
            </td>
        </tr>
    </#list>
<#else>
...
<#if files??>
<p style="color: #333;">下载数据文件后，请通过解密工具还原文件后使用。</p>
</#if>
