...
<script>
    ...
    var opFormat = function (value, column, row) {
    	if(row.canOperate == 1){
    	   return ["<a class='btn-icon icon-syxg mr-5' href='${contextPath}/manage/approval/accountChangeDetail.html?${_csrf.parameterName}=${_csrf.token}&id=" + row.id + "&approvalType=9" + "'>审批</a>"];
    	}else{
    	   return ["<a class='btn-icon icon-syxg mr-5' href='${contextPath}/manage/accountChange/recordDetial.html?${_csrf.parameterName}=${_csrf.token}&requestId=" + row.id + "&approvalType=9" + "'>详情</a>"];   		
    	}
    };
    var action = "${contextPath}/manage/approval/search.html?${_csrf.parameterName}=${_csrf.token}&approvalType=9";
    var columns = [{name: "createTime", text: "最小值修改时间", format: "DateTimeFormat"},
        {name: "productName", text: "账户名称"},
    	{name: "productTypeName", text: "账户类型"},
        {name: "count", text: "最小额度",format: countFormator},
