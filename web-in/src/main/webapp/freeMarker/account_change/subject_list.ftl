...
<script>
    ...
    var opFormat = function (value, column, row) {
        return ["<a class='btn-icon icon-down mr-5' href='${contextPath}/manage/accountChange/subjectCharge.html?accountChangeRequestId=" + row.id + "'>审批充值</a>"];
    };
    var action = "${contextPath}/manage/accountChange/subjectSearch.html?${_csrf.parameterName}=${_csrf.token}";
    var columns = [{name: "createTime", text: "申请充值时间", format: "DateTimeFormat"},
        {name: "count", text: "充值额度(元)"},
