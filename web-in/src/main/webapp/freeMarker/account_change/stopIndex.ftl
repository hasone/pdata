...
<script>
    ...
    var opFormat = function (value, column, row) {
    	if(row.canOperate == 1){
    	   return ["<a class='btn-icon icon-syxg mr-5' href='${contextPath}/manage/approval/accountChangeDetail.html?${_csrf.parameterName}=${_csrf.token}&id=" + row.id + "&approvalType=11" + "'>审批</a>"];
    	}else{
    	   return ["<a class='btn-icon icon-syxg mr-5' href='${contextPath}/manage/accountChange/recordDetial.html?${_csrf.parameterName}=${_csrf.token}&requestId=" + row.id + "&approvalType=11" + "'>详情</a>"];   		
    	}
    };

    var action = "${contextPath}/manage/approval/search.html?${_csrf.parameterName}=${_csrf.token}&approvalType=11";
    var columns = [{name: "createTime", text: "修改暂停值时间", format: "DateTimeFormat"},
        {name: "productName", text: "账户名称"},
    	{name: "productTypeName", text: "账户类型"},
        {name: "count", text: "暂停值",format: countFormator},
