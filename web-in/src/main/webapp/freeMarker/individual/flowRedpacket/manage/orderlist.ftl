...
<head>
    ...
    <title>流量平台-订购报表</title>
    <meta name="keywords" content="流量平台 订购报表"/>
    <meta name="description" content="流量平台 订购报表"/>
...
<script>
    ...
    var buttonsFormat = function (value, column, row) {
        return "<a href='${contextPath}/manage/scredpacket/showOrderDetail.html?orderSystemNum=" + row.systemNum + "'>详情</a>";
    }

    var productSizeFormat = function (value, column, row) {        
        return row.productSize/1024 + "MB";        
    }
    ...
    var columns = [{name: "createTime", text: "订购时间", format: "DateTimeFormat"},
        {name: "expireTime", text: "到期时间", format: "DateTimeFormat"},
        {name: "mobile", text: "手机号"},
        {name: "productName", text: "产品名称"},
        {name: "productSize", text: "产品大小", format: productSizeFormat},
        {name: "price", text: "价格", format: priceFormat},
        {name: "errorMsg", text: "状态", tip:true},
        {name: "op", text: "操作", format: buttonsFormat}];

    var action = "${contextPath}/manage/scredpacket/searchOrderlist.html?${_csrf.parameterName}=${_csrf.token}";
    ...
    function createFile() {
        ...
        window.open("${contextPath}/manage/scredpacket/createOrderlistFile.html?mobile=" + mobile 
