    ...
    <div class="tools row ">
        <div class="col-sm-12 dataTables_filter text-right">
            <div id="searchForm" class="form-inline searchForm" id="table_validate">            
                <input type="hidden" class="form-control searchItem" id="orderSystemNum" name="orderSystemNum" value="${orderSystemNum!}">
...
<script>
    ...
    var columns = [{name: "createTime", text: "抢红包时间", format: "DateTimeFormat"},
        {name: "chargeMobile", text: "手机号"},
        {name: "size", text: "产品大小", format: sizeFormat},
        {name: "status", text: "状态", tip:true, format: statusFormat}];

    var action = "${contextPath}/manage/scredpacket/searchOrderDetail.html?${_csrf.parameterName}=${_csrf.token}";
    ...
    function createFile() {
        ...
        window.open(
                "${contextPath}/manage/scredpacket/createOrderDetailListFile.html?orderSystemNum=" + orderSystemNum 
