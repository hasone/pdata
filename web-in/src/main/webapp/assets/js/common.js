...
$(function () {
    $(".dropdown-menu").on("click", "a", function () {
        ...
        $("input", parent).eq(0).val($(this).parent().data("value")).blur();
    });
    ...
    var autoInput = $(".enterprise_autoComplete");
    if (autoInput.length) {
        autoInput.after('<span id="autoComplete" class="text-left"></span>');
        require(["react", "react-dom", "AutoComplete"], function (React, ReactDOM, AutoComplete) {
            var pathname = location.pathname;
            var index = pathname.indexOf("/", 1);
            var ctx = pathname.substring(0, index);
            var url = autoInput.data("url") || "/manage/enterprise/query.html";
            var autoComplete = renderWidget(React, ReactDOM, AutoComplete, {
                data: [],
                filterurl: ctx + "/" + url,
    ...
    $(".province-menu").append('<li data-value="" ><a href="#">全部地区</a></li>\
    ...
    $("[required]").each(function () {
        $(this).closest(".form-group").children("label").eq(0)
            .prepend("<span class='required-item'>*</span>");
    });
...
Date.prototype.Format = function (formatStr) {
    ...
    str =
        str.replace(/yy|YY/, (this.getYear() % 100) > 9 ? (this.getYear() % 100).toString() : '0'
                                                                                              + (this.getYear()
                                                                                                 % 100));

    str =
        str.replace(/MM/,
                    this.getMonth() > 9 ? (this.getMonth() + 1).toString() : '0' + (this.getMonth()
                                                                             +      1));
    ...
    str =
        str.replace(/mm/,
                    this.getMinutes() > 9 ? this.getMinutes().toString() : '0' + this.getMinutes());
...
if (jQuery.validator) {
    ...
    jQuery.validator.addMethod("price", function (value, element) {
        return this.optional(element) || (/^(([1-9]+\d*)|([1-9]+\d*\.\d{1,2})|(0\.\d{1,2}))$/.test(
                value));
    }, "请填写正确的价格，两位小数的正数");
    ...
    jQuery.validator.addMethod("enterpriseName", function (value, element, param) {
        return this.optional(element) || (/^[\u4E00-\u9FA5A-Za-z0-9_\(\)\-\（\）]+$/.test(value));
    }, "可输入中英文、数字、下划线、括号");
    ...
    jQuery.validator.addMethod("ZHspace", function (value, element, param) {
    ...
    jQuery.validator.addMethod("format2", function (value, element, param) {
        return this.optional(element) || (/^[A-Za-z0-9\_]+$/.test(value) || (value === "******")); //字母
                                                                                                   // 、数字、下划线
    }, "请输入字母数字或下划线");
    ...
    jQuery.validator.addMethod("strictPwd", function (value, element, param) {
        return this.optional(element)
               || (/^(?![a-zA-Z0-9]+$)(?![^a-zA-Z/D]+$)(?![^0-9/D]+$).{10,20}$/.test(value));
    ...
    jQuery.validator.addMethod("payLimit", function (value, element, param) {
        return this.optional(element) || (/^((([1-9][\d]*)(\.[\d][\d]))|(0(\.(0[1-9]|[1-9][\d]))))$/.test(value));
    }, "请输入正数，最多两位小数");
    jQuery.validator.addMethod("payLimitNega", function (value, element, param) {
        return this.optional(element) || (/^[-]?((([1-9][\d]*)(\.[\d][\d]))|(0(\.(0[1-9]|[1-9][\d]))))$/.test(value));
    }, "请输入最多两位小数");
    jQuery.validator.addMethod("contract", function (value, element, param) {
        return this.optional(element) || (/^[A-Za-z0-9_\[\]\(\)\-\（\）\【\】]+$/.test(value));
    }, "支持字母、数字、()、-、[]、长度不超过32个字符，其中特殊字符支持中英文输入");
    jQuery.validator.addMethod("payType", function (value, element, param) {
        return this.optional(element) || (/^[\u4E00-\u9FA5A-Za-z0-9]+$/.test(value));
    }, "只能输入汉字字母和数字");
    ...
    $.fn.packDatetime = function (options) {
        ...
        return this.datetimepicker(defaults).on('changeDate', function (ev) {
            ...
            $(this).parents(".datetime-group").find('div.date').each(function (index, element) {
                ...
                if ($(currentDom).attr('data-group') == "date-begin" && $(element)
                                                                            .attr('data-group')
                                                                        == "date-end") {
                    //console.log($(currentDom).attr('data-group'))
                    //开始日期变更 调整结束日期范围
                    ev.date ? $(element).datetimepicker('setStartDate',
                                                        currentYear + '-' + currentMonth + '-'
                                                        + (currentDate + adjustDay)) : $(element)
                        .datetimepicker('setStartDate', defaults.startDate)
                } else if ($(currentDom).attr('data-group') == "date-end" && $(element)
                                                                                 .attr('data-group')
                                                                             == "date-begin") {
                    //结束日期变更 调整开始日期范围
                    ev.date ? $(element).datetimepicker('setEndDate',
                                                        currentYear + '-' + currentMonth + '-'
                                                        + (currentDate - adjustDay)) : $(element)
                        .datetimepicker('setEndDate', defaults.endDate)
                }
            })
        })
    };
...
function hasPrompt() {
    //有错误提示或输入框不为空时  隐藏输入规则提示  --输入框增加 class=“hasPrompt”  输入框后增加输入规则提示信息  <span
    // class="prompt">规则内容</span>
    $('form').submit(function () {
        $('form input[type=text].hasPrompt, form input[type=password].hasPrompt, form textarea.hasPrompt')
 ...
 * 获取渲染数据并渲染
 */
function render(url, convertTable, id) {
    $.ajax({
               beforeSend: function (request) {
                   var token1 = $(window.parent.document).find("meta[name=_csrf]").attr("content");
                   var header1 = $(window.parent.document).find("meta[name=_csrf_header]")
 ...
 * 渲染页面
 */
function showForm(data, convertTable, id) {
    $(".form-group:not(.form-group .form-group)").hide();
    ...
    for (var key in data) {
        ...
        //没有找到的话,就尝试使用转换表里的key搜索
        if (node.length <= 0) {
            key = convertTable[key];
            node = $('#' + key);
        }
        ...
        node.closest('.form-group').children('label').html(data[oldKey]['name'] + '：');
        node.closest('.form-group').children('.prompt').html(data[oldKey]['description']);
        node.closest('.form-group').show();
    }
    ...
    for (var default_key in defaultValue) {
        var old_key = default_key;
        var node = $('#' + default_key, ':hidden');

        if (node.length <= 0) {
            var new_key = convertTable[default_key];
            node = $('#' + new_key, ':hidden');
        }

        var value = defaultValue[old_key];
        if (value.hasOwnProperty("custom") && value["custom"]) {
            value["func"](node);
        } else {
            node.val(defaultValue[old_key]());
        }
    }
}
...
var imageBase64 = function () {
    return "";
};//"data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACcAAAAaCAIAAAC7JdIRAAAAAXNSR0IArs4c6QAAAARnQU1BAACxjwv8YQUAAAAJcEhZcwAADsMAAA7DAcdvqGQAAAAZSURBVEhL7cEBAQAAAIIg/69uSEAAAMCBGgv8AAHTCLcjAAAAAElFTkSuQmCC";

var randomPhone = function () {
    return "138" + Math.floor(Math.random() * 100000000);
};

var randomDate = function (node) {
    if (node.length > 0 && node.children("input").length > 0) {
        node.children("input").first() && node.children("input").first().val("1999-12-31 00:00:00");
        node.children("input").last() && node.children("input").last().val("2099-12-31 00:00:00");
    }
};

var randomEmail = function () {
    return randomString() + "@1.1";
};

var fixedValue = function (fixedValue) {
    return function () {
        return fixedValue;
    }
};
...
var defaultValue = {
    "name": randomString,
    "cooperationTime": {"func": randomDate, "custom": true},
    "code": randomString,
    "enterpriseManagerName": randomString,
    "enterpriseManagerPhone": randomPhone,
    "customerManagerPhone": randomPhone,
    "customerManagerName": randomString,
    "district": {"func": fixedValue(1), "custom": true},
    "entName": randomString,
    "email": randomEmail,
    "phone": randomPhone,
    "licenceImage": imageBase64,
    "licenceTime": {"func": randomDate, "custom": true},
    "authorization": imageBase64,
    "identification": imageBase64,
    "identificationBack": imageBase64,
    "customerManagerEmail": randomEmail,
    "customerTypeId": {"func": fixedValue(1), "custom": true},
    "businessTypeId": {"func": fixedValue(1), "custom": true},
    "prdType": {"func": fixedValue(1), "custom": true},
    "payTypeId": {"func": fixedValue(1), "custom": true},
    "discount": {"func": fixedValue(1), "custom": true},
    "giveMoneyId": {"func": fixedValue(1), "custom": true},
    "ec": imageBase64,
    "customerFile": imageBase64,
    "contract": imageBase64,
    "image": imageBase64,
    "entCredit": {"func": fixedValue(0), "custom": true}
};
/*******************************
 * 以上为渲染相关的代码
 ********************************/
