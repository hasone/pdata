 ...
 */
public abstract class AbstractEcBossServiceImpl implements BossService {
    private static final Logger LOGGER = LoggerFactory.getLogger(AbstractEcBossServiceImpl.class);
    ...
    public BossOperationResult charge(Long entId, Long splPid, String mobile, String serialNum, Long prdSize) {
        ...
        String featrue = product.getFeature();
        String pltPrdCode = new Gson().fromJson(featrue, TmpFeatureClass.class).getCode();
        ...
        if (!updateRecord(serialNum, dataResp == null ? SerialNumGenerator.buildNullBossRespNum("shandong") : dataResp.getSystemNum(), serialNum)) {
            LOGGER.error("更新流水失败");
        }
