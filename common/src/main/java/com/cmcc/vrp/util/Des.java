...
public class Des {
     ...
     * 加密、解密key.
     */
    private static final String PASSWORD_CRYPT_KEY = "don't try to find me!";
    /**
    ...
    public static byte[] encrypt(byte[] data, String secretKey) {
        if (StringUtils.isBlank(secretKey) || data == null || data.length < 1) {
            return null;
        }
        try {
            return encrypt(data, secretKey.getBytes());
        } catch (Exception e) {
            LOGGER.error(e);
        }
        return null;
    ...
    public static byte[] decrypt(byte[] data, String secretKey) {
        ...
        try {
            return decrypt(data, secretKey.getBytes());
        } catch (Exception e) {
            e.printStackTrace();
            LOGGER.error(e);
        }
