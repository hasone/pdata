...
/**
 * @author JamieWu
 *         企业营业执照到期后，将企业状态回退至体验账户
 */
public class EnterpriseLicenceExpireJob implements Job {

    private static final Logger logger = LoggerFactory.getLogger(EnterpriseLicenceExpireJob.class);
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        if (enterprise == null) {
            logger.info("企业营业执照到期定时任务企业不存在，企业ID-" + pojo.getEntId());
            return;
        } else {
            /**
             * 1、修改企业状态：企业状态暂停，企业EC接口关闭
             * */
            if (enterprise.getDeleteFlag().equals(EnterpriseStatus.NORMAL.getCode())) {
                ...
                entStatusRecord.setOpDesc("企业营业执照到期，企业暂停");

                if (!entStatusRecordService.insert(entStatusRecord)) {
                    logger.info("企业营业执照到期暂停企业，生成企业状态变更记录失败，企业ID-" + pojo.getEntId());
                }
                ...
                if (enterprise.getInterfaceFlag().equals(InterfaceStatus.OPEN.getCode())) {//企业暂停时，如果企业EC接口开通，则关闭企业EC接口，生成EC操作记录               
                    ...
                    entECRecord.setOpDesc("企业营业执照到期，企业暂停、EC同步关闭");
                    entECRecord.setNowStatus(enterprise.getInterfaceFlag());

                    if (!entECRecordService.insert(entECRecord)) {
                        logger.info("暂停企业，生成企业EC变更记录失败，企业ID-" + pojo.getEntId());
                    }
                }

                if (enterprisesService.updateByPrimaryKeySelective(enterprise)) {
                    logger.info("企业营业执照到期更新企业表状态为暂停成功，EC状态暂停成功，企业ID-" + pojo.getEntId());
                } else {
                    logger.info("企业营业执照到期更新企业表状态为暂停失败，企业ID-" + pojo.getEntId());
                }
            } else {
                logger.info("企业营业执照到期，此时企业处于非正常状态deleteFlag={}，不进行企业状态修改为暂停。企业ID-{}", enterprise.getDeleteFlag(),
                        pojo.getEntId());
            }

            /*//企业营业执照到期，所有全部进行中的活动都进行下架处理
